...
Keyword and competitor research means hitting YouTube data, and a hard rule applies: **no scraping from your local machine.** Pull research data through:
...
- **Managed scraping platforms** — if a bespoke pull is needed, run it on a managed scraping platform, never a local crawler.

If a request turns into "build me a YouTube scraper," that is a scraping/data-engineering job (tool ladder, managed APIs, anti-bot) — hand it off. This page only specifies *what* to research, not how to engineer the pull.

### What to research

1. **Seed → expand.** Start from the asset's value proposition ("scrape prices from site X", "tool X vs tool Y", "build an MCP server"). Expand with autosuggest and a keyword tool into the real phrasings people search.
...
4. **Demand vs. winnability.** A lower-volume query you can rank #1 for, with sharp intent, beats a high-volume query buried on page 3. Niche queries are often exactly this — modest volume, sharp intent, weak competition.
...
- All link prose should be humanized; tracked links carry your attribution parameter.
...
5. Description: value + core phrase + link above the fold (not a robotic raw-URL dump); real context below; humanized prose + attribution param.
