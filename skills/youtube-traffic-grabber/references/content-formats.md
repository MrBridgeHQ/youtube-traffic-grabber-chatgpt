...
This skill assumes **faceless content only** — no on-camera presenter, ever. This is not a limitation to work around; it is the operating constraint that every format here respects. Everything in the discovery doctrine (`youtube-traffic-doctrine.md`) and SEO (`youtube-seo.md`) still applies; the only thing removed is the talking head.

There are three faceless formats, mapped across the channels:

| Format | Channel | Best for |
|---|---|---|
| Screen-recorded dev tutorial / demo | dev (tool/docs site + tool listings) | search-intent how-tos: scraping, MCP, tooling |
| Data-viz / slideshow + voiceover | consumer (content/e-commerce sites) | explainers, rankings, market data |
...
- **Each video must teach or reveal something real** you actually know — a working technique, real market data, a genuine comparison. The dev tutorials come from your actual technical expertise; the data explainers from real, sourced data.
...
- **Voiceover and scripts read as human.** All script/voiceover prose should be humanized — this is both an authenticity and an AI-detection-risk safeguard. A script that reads as mass-generated is a liability.
...
- There's a matching asset (a tool, a docs/solution page) the tutorial naturally leads to.
...
Often these can be **lightly scripted or unscripted** when you know the material cold (the strongest practitioner case studies work this way) — bullet outline, record the screen, narrate live, then cut pauses. Don't over-engineer.
...
Narrated decks, charts, ranking/list videos, explainer animations. The consumer channel's backbone: market or ratings *data* is inherently visual and authoritative — price movements, regional comparisons, rankings — and it explains itself well over a clean slide with a voiceover. The auto-captioned voiceover also feeds search context (`youtube-seo.md`).

### When to use

- The value is **information/insight**, not a live procedure: "the 10 best-value options right now," "what this index actually tracks," "is this overpriced?"
- You have real, sourced data to show (pulled via the Data API / managed tools / a managed scraping platform — never local scraping).
- Informational or comparison intent — top-of-funnel and warm, building brand for the consumer properties.

### Scripting framework

```
1. HOOK (0:00–0:15)   A provocative number, claim, or question on screen.
                      "This product outscores ones 4x its price. Here's the data."
...
All voiceover prose should be humanized (in the channel's language — language segmentation is a real axis, not just niche).
...
Faceless Shorts: screen-capture clips, b-roll + on-screen text, data snippets, teasers of the long-form. The **cheapest way to reach new viewers**, on every channel.
...
- **Humanize the prose.** Every hook, script, and voiceover — EN and FR.
- **Each format earns the view before the link.** The CTA system (placement, attribution param, lead magnets) is in `traffic-conversion.md`; the destination must be a relevant page, not the bare homepage.
