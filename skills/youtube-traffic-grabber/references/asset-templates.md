...
- **YouTube links are nofollow - traffic, not SEO equity.** These templates drive clicks-to-site, conversions and brand recall. They do **not** build link equity; actual link-building is a separate off-page discipline.
- **Tracked links carry your attribution/affiliate parameter.** Every single one. A clean link forfeits referral attribution.
...
> **Humanize the prose before publishing (AI-detection-safe).** This appears under each template as a one-line reminder. Adapt the template, localize it, *then* humanize, *then* publish. Verbatim AI-flavored scripts and descriptions are exactly the "mass-produced / inauthentic" pattern YouTube's spam policy targets.

Placeholders use `{{double-braces}}`. Anything in `[square brackets]` is an authoring note - delete it before publishing.

**Channel mapping (illustrative):** dev/tech channel → tool/docs site + tool listings (EN primary). Consumer channels → an EN content property and a FR content/e-commerce property. Keep distinct niches separate; resolve language-split per `channel-strategy.md`.
...
- HOOK (0:00–0:15) - say the payoff first, show the end result on screen - {{One sentence naming the exact outcome the viewer searched for: "Here's how to get clean {{X}} data without writing your own scraper."}}
[On screen: the finished result / the structured output, before you explain anything.]

- PROBLEM (0:15–0:45) - name the pain, briefly, so the demo feels necessary - {{1–2 sentences on why the obvious approach is painful: e.g. "Most people reach for a headless browser and end up babysitting proxies and broken selectors."}}
[Optional: show the broken/slow way for 3–5s as contrast. Don't dwell - momentum.]

- DEMO (0:45–~end) - deliver the full method on screen, step by step - {{Step 1 - concrete, narrated as you do it on screen.}}
...
- CTA (last 20–30s) - value-tied, single, non-pushy - {{Verbal + on-screen: "If you'd rather not maintain the session and proxy layer yourself, I left a link to the tool that does exactly this in the description."}}
[On screen end-card: the destination. Card mid-video only if a link is genuinely relevant at that moment.]
```

Filled (tool demo, dev channel):
...
PROBLEM: "Most tutorials spin up Playwright and then spend a week fighting rate limits and rotating tokens. You don't need a browser for this at all."

DEMO: [Show the internal JSON endpoint in DevTools → replay with the region param → structured rows. Then show the maintained tool doing the same hands-off.]
...
[Description link → https://example.com/price-scraper?fpr=YOUR_ID]
```

> Humanize the prose before publishing (AI-detection-safe).
...
- ACCROCHE (0:00–0:15) - annoncer le résultat d'abord, le montrer à l'écran - {{Une phrase nommant le résultat exact recherché : « Voici comment récupérer des données {{X}} propres sans coder son propre scraper. »}}
[À l'écran : le résultat final / la sortie structurée, avant toute explication.]

- PROBLÈME (0:15–0:45) - nommer la douleur, brièvement - {{1–2 phrases : pourquoi l'approche évidente est pénible.}}

- DÉMO (0:45–~fin) - livrer toute la méthode à l'écran, étape par étape - {{Étape 1 - concrète, narrée pendant qu'on la fait.}}
...
- CTA (20–30 dernières s) - lié à la valeur, unique, sans forcer - {{Voix + texte à l'écran : « Si tu préfères ne pas maintenir tout ça toi-même, le lien est en description. »}}
```

> Humaniser la prose avant publication (sûr face à la détection IA).

---

## 2. Faceless data explainer script (data-viz / slideshow)

**When:** a narrated deck / data-viz / ranking video - charts, list videos, market explainers built on real, sourced data. Faceless by nature: slides + voiceover. The spoken keywords feed the auto-caption and the search context, so narrate the topic plainly. **FR for the FR-language property; EN for the EN-language property.** Same hook→problem→payoff→CTA spine, slower and more editorial than the dev demo.

### EN (EN-language consumer property)

```
[Slideshow / data-viz + voiceover. No presenter. Each slide = one idea.]

- HOOK (0:00–0:15) - the surprising data point first - {{One striking, true number on screen: "Secondary-market prices in this category moved {{X}}% in {{period}} - here's what the data actually shows."}}

- SETUP (0:15–0:45) - why a single price misleads - {{1–2 sentences: a single retailer's price is a snapshot; aggregated market data gives the real curve.}}

- PAYOFF (0:45–~end) - walk the charts, one insight per slide - {{Chart 1 - the trend, narrated.}}
...
- CTA (last 20–30s) - value-tied - {{"If you want the full pricing breakdown behind these charts, it's linked in the description."}}
[Destination = the relevant page, not the homepage.]
```

> Humanize the prose before publishing (AI-detection-safe).

### FR (FR-language consumer property)

```
[Diaporama / data-viz + voix off. Pas de présentateur. Une slide = une idée.]

- ACCROCHE (0:00–0:15) - le chiffre qui surprend, d'abord - {{Un chiffre vrai et marquant à l'écran : « Le prix moyen dans cette catégorie a bougé de {{X}}% en {{période}} - voici ce que disent vraiment les données. »}}

- MISE EN PLACE (0:15–0:45) - pourquoi un prix isolé trompe - {{1–2 phrases : un prix isolé est un instantané ; les données marché donnent la vraie courbe.}}

- LE FOND (0:45–~fin) - dérouler les graphes, une idée par slide - {{Graphe 1 - la tendance, narrée.}}
{{Graphe 2 - la comparaison / le classement qui rend ça concret.}}
{{Graphe 3 - le conseil actionnable pour l'acheteur.}}
[Citer la source des données à l'écran - l'exactitude est non négociable, aucun chiffre gonflé.]

- CTA (20–30 dernières s) - lié à la valeur - {{« Si tu veux le détail derrière ces graphes, le lien est en description. »}}
...
- HOOK (0:00–0:02) - the question or payoff, on screen as text too - {{"You don't need a headless browser to scrape {{site}}."  /  "{{Category}} just did something weird in the data."}}

- PAYOFF (0:02–0:50) - deliver the one idea, fast - {{The single insight or the one-step trick. On-screen text mirrors the voiceover. One visual beat ~every 5–10s.}}

- LOOP / CTA (0:50–0:60) - point to the long-form, not a hard link - {{"Full breakdown on the channel."  /  "Long version's pinned in the comments."}}
...
- ACCROCHE (0:00–0:02) - la question ou le résultat, aussi en texte à l'écran - {{« Pas besoin de navigateur headless pour scraper {{site}}. »  /  « Cette {{catégorie}} a fait un truc bizarre dans les données. »}}

- LE FOND (0:02–0:50) - livrer l'idée unique, vite - {{L'insight unique ou l'astuce en une étape. Le texte à l'écran double la voix off. Un temps visuel toutes les ~5–10s.}}

- BOUCLE / CTA (0:50–0:60) - renvoyer vers le long format, pas un lien dur - {{« Version complète sur la chaîne. »  /  « Le long format est épinglé en commentaire. »}}
```
...
Filled: `How to scrape product prices without a headless browser` · `This category's prices moved 14% this quarter - here's what the data shows`
...
- {{Catégorie}} : ce que les données disent vraiment des prix
- Le détail que personne ne dit sur {{sujet}}
```

> Humanize the prose before publishing (AI-detection-safe). / Humaniser la prose avant publication.
...
👉 {{What the link gives them}}: {{funnel URL - tracked links carry your attribution param}}
...
Filled above-the-fold link: `👉 The tool that does this hands-off: https://example.com/price-scraper?fpr=YOUR_ID`
...
👉 {{Ce que le lien apporte}} : {{URL de funnel - les liens suivis portent ton paramètre d'attribution}}
...
{{What/why the link}}: {{funnel URL - attribution param on tracked links}}
[Optional: invite a real question - "What would you want covered next?"]
```

Filled: `Heads-up: set your region parameter or you'll get mixed currencies. Hands-off version: https://example.com/price-scraper?fpr=YOUR_ID - what site should I cover next?`

### FR

```
{{Un ajout réellement utile : un bémol, une astuce de suivi, ou la réponse à la question que la vidéo soulève.}}
{{Quoi/pourquoi le lien}} : {{URL de funnel - paramètre d'attribution sur les liens suivis}}
...
(Full disclosure: I build {{property}}.) {{One sentence on how it solves the exact thing discussed}} - {{funnel URL, attribution param on tracked links}}. The manual way works fine too.
```

Filled: `Good walkthrough. One thing that saved me a ton of time: this site serves prices from an internal JSON endpoint, so you can skip the browser entirely. (Full disclosure: I build scraping tools.) If you'd rather not maintain the session layer: https://example.com/price-scraper?fpr=YOUR_ID - but the DevTools route is free.`
...
(Transparence : je développe {{propriété}}.) {{Une phrase sur comment ça résout exactement le sujet}} - {{URL de funnel, paramètre d'attribution sur les liens suivis}}. La méthode manuelle marche aussi.
```
...
These are EN + FR starters. If your sites are multilingual and the niche spans languages, a canned translation posted verbatim reads as inauthentic and underperforms. To extend a market: keep the structure, rewrite idiomatically in the target language (the actual word a native uses), match the channel's language to its audience, then humanize the result. All non-negotiables hold in every language: earned views, faceless, nofollow-traffic framing, every tracked link carrying your attribution param, paid promo disclosed.
...
6. **Tracked URL?** Carries your attribution/affiliate parameter. No bare link.
7. **nofollow understood?** This is traffic/brand, not link equity. Link-building → a dedicated off-page method.
