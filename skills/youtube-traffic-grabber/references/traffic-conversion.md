...
Pick the destination by audience and language: the FR-language property for FR intent, the EN-language property for EN/international intent (`channel-strategy.md` resolves the language/property split).
...
- **Property→channel mapping, FR/EN destination split** → `channel-strategy.md`
...
- **KPIs: clicks-to-site, conversions via the attribution param, per-placement CTR** → `measurement.md`
...
- **Acquiring SEO link equity (not what YouTube links do)** → a dedicated link-building method
