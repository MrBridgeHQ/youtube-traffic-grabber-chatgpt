...
YouTube is a traffic, brand, and discovery channel - not an SEO-equity channel. Every outbound link it carries is `nofollow`. The job is not "get a link from YouTube"; it is "get the right viewer to watch, then earn the click off-platform to one of your own assets." This page is the directing frame for everything else in the skill.
...
search / suggested / browse  ──►   CTA (description link, pinned comment,
   ...
   retention / AVD             off-platform click → your asset
        │                              │
        ▼                              ▼
   the video gets watched      convert / lead / brand recall
```
...
**The destination is the multiplier.** Placements only move clicks if there is a compelling, *relevant* destination - a specific landing page, the matching tool, a data explainer that continues the video - not the bare homepage. Match the offer to the video topic.

Every tracked link carries your attribution/affiliate parameter. All CTA prose, scripts, and descriptions should be humanized so they read natural.
...
- **Do not pursue YouTube as a link-building tactic.** Pursue it for clicks, leads, conversions, and brand recall. Off-page authority work - acquiring dofollow links, anchor-text strategy, link velocity - is a separate link-building discipline. If a request is really "I want backlinks for my site," that is a link-building job, not this skill's.
...
YouTube earns you watched videos from interested humans, then a nofollow click to a relevant asset - traffic, brand, and discovery, never PageRank. Optimize for the watch (CTR + retention), then for the click (native, value-first CTA to a specific destination), and route anything that is actually about link equity to a dedicated link-building method.
